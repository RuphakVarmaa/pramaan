...
All numbers below are generated by `npm run batch` (60 seeded scenarios, 25 in-scope / 15 out-of-scope / 10 disputed / 5 flagged-legit / 5 flagged-malicious; PRNG seed committed in the report). Regenerate them yourself with that command. _(pending: swarm S5 — `npm run batch` + `metrics/summary.md` land with the batch runner)_

| Metric | Definition (CONTRACTS.md §9) | Value |
|---|---|---|
| In-scope pass rate | in-scope scenarios ending `PAYMENT_CAPTURED` ÷ 25 | _(pending: npm run batch)_ |
| Out-of-scope block rate | out-of-scope scenarios ending blocked with a reason code ÷ 15 | _(pending: npm run batch)_ |
| Evidence latency | median ms, dispute → dossier, over 10 disputed scenarios | _(pending: npm run batch)_ |
| Legit release rate | flagged-legit scenarios ending `AGENT_RELEASED` ÷ 5 | _(pending: npm run batch)_ |
| Malicious block rate | flagged-malicious scenarios ending blocked ÷ 5 | _(pending: npm run batch)_ |
| False-positive cost, before | Σ blocked-but-legit amountPaise under risk-engine-only policy | _(pending: npm run batch)_ |
| False-positive cost, after | Σ blocked-but-legit amountPaise under Pramaan pass-through | _(pending: npm run batch)_ |

Raw output lands in `metrics/report.json`, the human summary with exceptions in `metrics/summary.md`. Failures and unresolved cases are reported there, not hidden — that is the point of the exercise.
