...
Issue a delegation, then try an in-scope checkout, a cap-exceeded refusal, a dispute, the evidence pack, and both fraud verdicts — every endpoint from JUDGE.md, live.

**The console** (same URL, in a browser) ships in two modes:

- **MOCK ENGINE** (default) — the fully self-contained demo: no backend needed, identical state machine, perfect for a quick tour or recording the video
- **GO LIVE** — the same UI talking to the *real* backend: every issuance is genuinely Ed25519-signed, every checkout runs the real gate and Razorpay test rails, every evidence pack is generated by the production dossier engine

**Serverless persistence:** the deployment uses a deterministic demo signer (`PRAMAAN_SIGNING_SEED`), a `/tmp`-backed SQLite ledger, and an artifact sidecar — so delegations verify across container cold starts. It is still a demo deployment (best-effort persistence, seeded demo key, stub payments unless test keys are set as env vars — `/health` always names the mode). The production deployment is the Dockerfile: volume-mounted ledger, merchant-held keys, KMS.
